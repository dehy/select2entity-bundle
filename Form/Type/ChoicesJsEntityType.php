...
namespace Dehy\ChoicesJsEntityBundle\Form\Type;
...
use Dehy\ChoicesJsEntityBundle\Form\DataTransformer\EntitiesToPropertyTransformer;
use Dehy\ChoicesJsEntityBundle\Form\DataTransformer\EntityToPropertyTransformer;
use Symfony\Component\PropertyAccess\PropertyAccess;

/**
 *
 * Class ChoicesJsEntityType
 * @package Dehy\ChoicesJsEntityBundle\Form\Type
 */
class ChoicesJsEntityType extends AbstractType
{
    /** @var ObjectManager */
    protected $em;
    /** @var RouterInterface */
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if (isset($options['object_manager'])) {
            ...
            if (!$em instanceof ObjectManager) {
                throw new Exception(
                    'The entity manager \'em\' must be an ObjectManager instance'
                );
            }
        ...
        if ($options['transformer']) {
            ...
            if (!class_exists($options['transformer'])) {
                throw new Exception(
                    'Unable to load class: ' . $options['transformer']
                );
            }
            ...
            $newTagPrefix = isset($options['allow_add']['new_tag_prefix'])
                ? $options['allow_add']['new_tag_prefix']
                : $this->config['allow_add']['new_tag_prefix'];
            $newTagText = isset($options['allow_add']['new_tag_text'])
                ? $options['allow_add']['new_tag_text']
                : $this->config['allow_add']['new_tag_text'];
    ...
    public function finishView(
        ...
        $view->vars['remote_path'] =
            $options['remote_path'] ?:
            $this->router->generate(
                $options['remote_route'],
                array_merge($options['remote_params'], [
        ...
        $varNames = array_merge(
            array('multiple', 'placeholder', 'primary_key', 'autostart'),
        ...
        if (
            ...
            foreach ($options['req_params'] as $key => $reqParam) {
                $reqParams[$key] = $accessor->getValue(
                    $view,
                    $reqParam . '.vars[full_name]'
                );
            }
        ...
        foreach ($varNames as $varName) {
            if (isset($options['allow_add'][$varName])) {
                $view->vars['allow_add'][$varName] =
                    $options['allow_add'][$varName];
            } else {
                $view->vars['allow_add'][$varName] =
                    $this->config['allow_add'][$varName];
            }
        }
